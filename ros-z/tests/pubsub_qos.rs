...
mod tests {
    ...
    /// Test pub/sub with Volatile durability and Reliable reliability across different contexts
    /// This uses AdvancedPublisher without cache
    /// This now WORKS after fixing reliability setting and session discovery
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_pubsub_volatile_reliable() -> Result<()> {
        let (_ctx1, pub_node) = setup_test_node("test_pub_volatile_reliable").await?;
        let (_ctx2, sub_node) = setup_test_node("test_sub_volatile_reliable").await?;
    ...
    async fn test_pubsub_volatile_best_effort() -> Result<()> {
        let (_ctx1, pub_node) = setup_test_node("test_pub_volatile_best_effort").await?;
        let (_ctx2, sub_node) = setup_test_node("test_sub_volatile_best_effort").await?;
    ...
    /// Test pub/sub with TransientLocal durability and Reliable reliability across contexts
    /// This uses AdvancedPublisher WITH cache
    /// NOTE: Cache history retrieval may not work across different contexts
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_pubsub_transient_local_reliable() -> Result<()> {
        let (_ctx1, pub_node) = setup_test_node("test_pub_transient_reliable").await?;
        let (_ctx2, sub_node) = setup_test_node("test_sub_transient_reliable").await?;
    ...
    async fn test_pubsub_transient_local_best_effort() -> Result<()> {
        let (_ctx1, pub_node) = setup_test_node("test_pub_transient_best_effort").await?;
        let (_ctx2, sub_node) = setup_test_node("test_sub_transient_best_effort").await?;
    ...
    /// Test pub/sub with KeepAll history (across contexts)
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_pubsub_keep_all_history() -> Result<()> {
        let (_ctx1, pub_node) = setup_test_node("test_pub_keep_all").await?;
        let (_ctx2, sub_node) = setup_test_node("test_sub_keep_all").await?;
    ...
    fn test_sync_advanced_publisher_no_runtime() -> Result<()> {
        ...
        let ctx2 = ros_z::context::ZContextBuilder::default().build()?;
        let pub_node = ctx1.create_node("sync_test_pub").build()?;
        let sub_node = ctx2.create_node("sync_test_sub").build()?;
    ...
    fn test_sync_publish_with_deadline_liveliness() -> Result<()> {
        ...
        let ctx1 = ros_z::context::ZContextBuilder::default().build()?;
        let ctx2 = ros_z::context::ZContextBuilder::default().build()?;
        let pub_node = ctx1.create_node("sync_deadline_pub").build()?;
        let sub_node = ctx2.create_node("sync_deadline_sub").build()?;
